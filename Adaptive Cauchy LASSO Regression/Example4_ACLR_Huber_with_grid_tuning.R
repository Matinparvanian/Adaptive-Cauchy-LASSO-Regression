...
# Non-Cauchy noise:
#   eps_i ~ S(alpha, 0, 1, 0), alpha in {1.5, 2}
# alpha = 2 corresponds to Gaussian noise in the stable family.
#
# beta = (4,4,4,4,0,0,0,0,0,0)^T

p <- 10
beta_true <- c(4, 4, 4, 4, rep(0, 6))
...
# Symmetric alpha-stable noise S(alpha, 0, 1, 0)
# Using Nolan parameterization through stabledist::rstable(..., pm = 1)
generate_stable_noise <- function(n, alpha) {
  stabledist::rstable(
    n = n,
    alpha = alpha,
    beta = 0,
...
generate_example4_data <- function(n_train, n_test, p, rho, beta_true, alpha) {
  ...
  eps_train <- generate_stable_noise(n_train, alpha)
  eps_test  <- generate_stable_noise(n_test, alpha)
...
make_result_row <- function(n_train, rho, alpha, method, beta_hat, y_test, pred_test,
...
###################################
# 8) one repetition for all methods
##################################

run_one_replication_all_methods <- function(n_train, n_test, rho, alpha, beta_true,
                                            lambda_grid_clr,
                                            lambda_grid_ql, lambda_grid_aclr,
                                            adaptive_gamma, adaptive_eps,
                                            huber_k, lambda_init_huber_ridge) {
  dat <- generate_example4_data(
    n_train = n_train,
    n_test = n_test,
    p = length(beta_true),
    rho = rho,
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "AIC-CLR",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "BIC-CLR",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "AIC-ACLR-Huber",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "BIC-ACLR-Huber",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "CV-LASSO",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "QL-0.25",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "QL-0.5",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "QL-0.75",
  ...
  out_rows[[length(out_rows) + 1]] <- make_result_row(
    n_train = n_train, rho = rho, alpha = alpha, method = "NOR",
...
run_one_replication_aclr_only <- function(n_train, n_test, rho, alpha, beta_true,
                                          lambda_grid_aclr,
                                          adaptive_gamma, adaptive_eps,
                                          huber_k, lambda_init_huber_ridge) {
  dat <- generate_example4_data(
...
cat("Starting ACLR hyperparameter tuning for Example 4...\n")
...
utils::write.csv(aclr_tuning_summary, "aclr_tuning_summary_example4.csv", row.names = FALSE)
utils::write.csv(best_aclr_setting, "best_aclr_setting_example4.csv", row.names = FALSE)
  ...
  for (n_train in n_train_values) {
    for (rho in rho_values) {
      for (b in 1:n_rep) {
        ...
        if (b %% report_every == 0) {
          cat("alpha =", alpha, "n_train =", n_train, "rho =", rho, "rep =", b, "of", n_rep, "\n")
        }
      }
    }
  }
...
results_all$alpha <- factor(results_all$alpha, levels = c(1.5, 2))
...
plot_mae <- ggplot(results_mae, aes(x = method, y = mae_test, fill = factor(rho))) +
  geom_boxplot() +
  facet_grid(alpha ~ n_train, scales = "free_y") +
  labs(x = "Method", y = "Test MAE", fill = expression(rho)) +
  theme_bw(base_size = 12)
print(plot_mae)

mae_heatmap_data <- results_mae %>%
  group_by(alpha, n_train, rho, method) %>%
  summarise(mean_mae = mean(mae_test), .groups = "drop") %>%
  mutate(scenario = paste0("alpha=", alpha, ", n=", n_train, ", rho=", rho))
...
mae_table <- results_mae %>%
  ...
  select(alpha, rho, n_train, method, mae_text) %>%
...
out_dir <- "Example2_ACLR_Huber_results"
if (!dir.exists(out_dir)) {
  dir.create(out_dir, recursive = TRUE)
}

write.csv(results_all, file.path(out_dir, "example2_all_methods_results_full.csv"), row.names = FALSE)
write.csv(fnr_table, file.path(out_dir, "example2_fnr_table.csv"), row.names = FALSE)
write.csv(fpr_table, file.path(out_dir, "example2_fpr_table.csv"), row.names = FALSE)
write.csv(mae_table, file.path(out_dir, "example2_mae_table_with_NOR.csv"), row.names = FALSE)
write.csv(training_time_table, file.path(out_dir, "example2_training_time_minutes.csv"), row.names = FALSE)
write.csv(selection_summary, file.path(out_dir, "example2_selection_summary.csv"), row.names = FALSE)
write.csv(mae_summary, file.path(out_dir, "example2_mae_summary.csv"), row.names = FALSE)

ggsave(file.path(out_dir, "example2_selected_boxplot.png"), plot_selected, width = 14, height = 8, dpi = 300)
ggsave(file.path(out_dir, "example2_unrelated_boxplot.png"), plot_unrelated, width = 14, height = 8, dpi = 300)
ggsave(file.path(out_dir, "example2_mae_boxplot_with_NOR.png"), plot_mae, width = 14, height = 8, dpi = 300)
ggsave(file.path(out_dir, "example2_mae_heatmap.png"), plot_mae_heatmap, width = 12, height = 8, dpi = 300)
